            ...
            ConfigError::MissingField(field) => {
                ...
                match *field {
                    "AWS_REGION" => {
                        ...
                        writeln!(f, "Pls provide it via --region or AWS_REGION environment variable.")?;
                        writeln!(f)?;
                        writeln!(f, "Note: If you see IMDS timeout warnings above, it means the program")?;
                        writeln!(f, "      attempted to auto-discover the region from EC2 metadata but")?;
                        writeln!(f, "      you are not running on AWS infrastructure.")?;
                    }
                    "AWS_SECRET_ACCESS_KEY" => {
                        writeln!(f, "AWS credentials must be provided as a complete pair.")?;
                        writeln!(f)?;
                        writeln!(f, "You provided AWS_ACCESS_KEY_ID but not AWS_SECRET_ACCESS_KEY.")?;
                        writeln!(f)?;
                        writeln!(f, "Alternatively, omit both to use AWS credential provider chain")?;
                        writeln!(f, "(IAM roles, instance profiles, etc.).")?;
                    }
                    "AWS_ACCESS_KEY_ID" => {
                        writeln!(f, "AWS credentials must be provided as a complete pair.")?;
                        writeln!(f)?;
                        writeln!(f, "You provided AWS_SECRET_ACCESS_KEY but not AWS_ACCESS_KEY_ID.")?;
                        writeln!(f)?;
                        writeln!(f, "Alternatively, omit both to use AWS credential provider chain")?;
                        writeln!(f, "(IAM roles, instance profiles, etc.).")?;
                    }
                    ...
                    "SERVICE_ACCESS_TOKEN" => {
                        writeln!(f, "Service access token is required for client authentication.")?;
                        writeln!(f)?;
                        ...
                        writeln!(f)?;
                        writeln!(f, "This token must match the token configured in your Nx clients.")?;
                    }
                    _ => {
